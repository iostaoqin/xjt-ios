//
//  UDIDDriverLicenseEngine.h
//  UubeeSuperReal
//
//  Created by JiMac on 2018/10/10.
...
@protocol UDIDDriverLicenseEngineDelegate <NSObject>

/**
 * 返回结果，cancel是用户取消操作，Done 是完成检测 ，userInfo是完成检测之后的返回信息
 **/

- (void)driverLicenseFinishedWithResult:(NSInteger)result userInfo:(id)userInfo; // swift

- (void)driverLicenseFinishedWithResult:(UDIDSafeAuthResult)result UserInfo:(id)userInfo; //oc

@end

@interface UDIDDriverLicenseEngine : NSObject

@property (weak, nonatomic) id <UDIDDriverLicenseEngineDelegate> delegate;
 ...
 */
- (void)startDriverLicenseAuthInViewController:(UIViewController *)viewController;
