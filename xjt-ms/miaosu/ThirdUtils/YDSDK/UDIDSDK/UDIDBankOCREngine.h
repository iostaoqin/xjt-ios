...
@protocol UDIDBankOcrEngineDelegate <NSObject>

/**
 * 返回结果，cancel是用户取消操作，Done 是完成检测 ，userInfo是完成检测之后的返回信息
 **/

- (void)bankOcrFinishedWithResult:(NSInteger)result userInfo:(id)userInfo; // swift

- (void)bankOcrFinishedWithResult:(UDIDSafeAuthResult)result UserInfo:(id)userInfo; //oc

@end

@interface UDIDBankOCREngine : NSObject

@property (weak, nonatomic) id <UDIDBankOcrEngineDelegate> delegate;
 ...
 */
- (void)startBankOcrAuthInViewController:(UIViewController *)viewController;
